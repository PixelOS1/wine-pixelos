...
params=$(getopt -n $0 -o a:b:w:v:p:z:d:sn6kfcmt --long as:,branch:,with:,version:,patch:,mingwportablepath:,dependencies:,staging,noupload,64bit,keep,keep-upload-file,useccache,usemingw,nostrip -- "$@")
eval set -- $params
while true ; do
    case "$1" in
